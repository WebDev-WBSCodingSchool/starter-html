...
// `gh` is pointed at the template from WBS CODING SCHOOL instead of the group's own.
//
...
if (has("--check")) {
  ...
  try {
    ...
    if (!mine) {
      ...
      console.log(`    That is the template from WBS CODING SCHOOL, not your group's copy. Create your`);
      console.log(`    own repo from it ("Use this template" on GitHub), add your group as`);
      console.log(`    collaborators, and clone that — your commits here have nowhere to push,`);
      console.log(`    and a Pull Request from here goes to somebody else's repository.`);
      failed = true;
    } else {
    ...
    console.log(`    Make sure you cloned your group's own repo, not the template from WBS CODING SCHOOL.`);
  }
...
if (has("--issues")) {
  ...
  //
  // The Issues tab may also be off. The template from WBS CODING SCHOOL ships it off, and
  // a repo created from a template does not necessarily inherit that setting —
  // so this does not assume either way, it reads the flag and fixes it if needed.
  let repo = null;
  try {
